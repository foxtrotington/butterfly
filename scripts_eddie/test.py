...
butterflyTaxonIds = pd.read_csv("../data/in/species.csv")
...
def getButterflyDataFrom(pathToCsv):
    ...
    butterflyData.to_csv("../data/in/inat_butterfly_observations.csv")
...
def getAccessToken():
    ...
    with open('../data/out/observations_from_inat_api.txt', 'w') as outfile:
        json.dump(json.loads(obs_data.text), outfile)

def main():
    getButterflyDataFrom("../data/in/observations.csv")
    #getAccessToken()
